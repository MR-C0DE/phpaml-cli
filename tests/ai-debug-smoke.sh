...
mkdir -p "$fixture/runtime/public" \
  "$fixture/runtime/aml_env/storage/debug-reports" \
  "$fixture/runtime/aml_env/storage/debug-backups/20260812-120000-a1b2c3/public"
printf '%s\n' '<?php echo "current";' > "$fixture/runtime/public/index.php"
printf '%s\n' '<?php echo "original";' > "$fixture/runtime/aml_env/storage/debug-backups/20260812-120000-a1b2c3/public/index.php"
cat > "$fixture/runtime/aml_env/storage/debug-reports/20260812-120000-a1b2c3.json" <<'JSON'
{
...
history="$(cd "$fixture/runtime" && HOME="$fixture/home" AML_LANG=en php aml_env/bin/aml.php debug:history)"
grep -q '20260812-120000-a1b2c3' <<<"$history"
details="$(cd "$fixture/runtime" && HOME="$fixture/home" AML_LANG=en php aml_env/bin/aml.php debug:show 20260812-120000-a1b2c3)"
grep -q 'Smoke diagnosis' <<<"$details"
(cd "$fixture/runtime" && HOME="$fixture/home" AML_LANG=en php aml_env/bin/aml.php \
  debug:rollback 20260812-120000-a1b2c3 --yes >/dev/null)
grep -q 'original' "$fixture/runtime/public/index.php"
grep -q '"status": "rolled_back"' "$fixture/runtime/aml_env/storage/debug-reports/20260812-120000-a1b2c3.json"
